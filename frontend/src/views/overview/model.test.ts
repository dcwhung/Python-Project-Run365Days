...
describe("overview model", () => {
  ...
  it("weatherStrip counts rainy, severe and finds the best temperature band", () => {
    const acts = [
      ...[20, 21, 22, 23, 23.5].map((t, i) =>
        act({ id: `w${i}`, date: `2021-03-0${i + 1}`, avgTempC: t, paceSecPerKm: 300 }),
      ),
      ...[28, 29, 30, 31, 31.5].map((t, i) =>
        act({ id: `h${i}`, date: `2021-06-0${i + 1}`, avgTempC: t, paceSecPerKm: 340 }),
      ),
      ...
      act({
        ...
        date: "2021-07-01",
        avgTempC: null,
        weather: { description: "Rain", tempC: 25, humidityPct: 90, windKmh: 5 },
        warnings: ["STRONG WIND SIGNAL NO. 3"],
      }),
    ];
    const weather = [
      { date: "2021-07-01", rainfallMm: 12 },
      { date: "2021-03-01", rainfallMm: 0 },
    ].map((w) => ({
      date: w.date,
      maxTempC: null,
      avgTempC: null,
  ...
  it("rolling7 averages the trailing week", () => {
    const daily = [1, 2, 3, 4, 5, 6, 7, 8].map((km, i) => ({
      date: `d${i}`,
      distanceKm: km,
      activityId: null,
    }));
