...
const wx = (description: string, tempC: number, humidityPct = 70) => ({
  description,
  tempC,
  humidityPct,
  windKmh: 10,
...
const ACTS = [
  ...
  act({
    ...
    avgTempC: 30,
    paceSecPerKm: 340,
    weather: wx("Rain", 29, 90),
    warnings: ["RED RAINSTORM WARNING SIGNAL", "THUNDERSTORM WARNING"],
  }),
  act({
    id: "c",
    date: "2021-01-03",
...
const day = (date: string, o: Partial<DailyWeather>): DailyWeather => ({
  date,
  maxTempC: null,
  avgTempC: null,
  minTempC: null,
...
const WEATHER = [
  day("2021-01-01", { maxTempC: 16, minTempC: 9, sunrise: "07:03" }),
  day("2021-01-02", { rainfallMm: 30, sunrise: "07:03" }),
];
...
describe("weather model", () => {
  ...
  it("temperature range joins HKO and the run", () => {
    const r = temperatureRange(
      [
        { date: "2021-01-01", activityId: "a" },
        { date: "2021-01-05", activityId: null },
      ],
      ACTS,
      WEATHER,
    );
  ...
  it("conditions, bands, humidity", () => {
    expect(conditions(ACTS).map((c) => [c.label, c.runs])).toEqual([
      ["Clear weather", 1],
      ["Rain", 1],
      ["Thunderstorm", 1],
    ]);
