...
describe("buildSeries", () => {
  it("derives pace, cadence, distance and gps flag", () => {
    const track = [
      pt({ sec: 0 }),
      pt({ sec: 10, speedMps: 2.5 }),
      pt({ sec: 20, cadence: 40, tempC: null }),
    ];
  ...
  it("fills missing coordinates from neighbours and flags indoor runs", () => {
    const track = [
      pt({ sec: 0, lat: null, lon: null }),
      pt({ sec: 5, lat: 1, lon: 2 }),
      pt({ sec: 10, lat: null, lon: null }),
    ];
    const s = buildSeries(act({ id: "a", date: "2021-01-01" }), track);
    expect(s.latLon).toEqual([
      [1, 2],
      [1, 2],
      [1, 2],
    ]);
    const indoor = buildSeries(act({ id: "b", date: "2021-01-02", hasGps: false }), [
      pt({ sec: 0, lat: null, lon: null }),
    ]);
  ...
  it("carries cumulative distance when a point lacks it", () => {
    const track = [
      pt({ sec: 0, distanceM: 0 }),
      pt({ sec: 5, distanceM: null }),
      pt({ sec: 10, distanceM: 40 }),
    ];
