...
**最後更新**：2026-09-16（**CUI-0027 / CUI-0022 / CUI-0024 批次 QA 完成 —— ✅ pass、0 Critical、hard gates 8/8**：Python **382 tests** / 前端 **126 tests（22 files）** / coverage **95% TOTAL**，`src/api/schema.py` 同 `src/common/time.py` 各 **100%**。真 `createApiSource` call path 打真 Flask server 八個 method 全過；budget 邊界實測（4 個 list field × `limit:1000` = 4000 **served**，+`year` = 4500 **refused**，前端餘裕 8×）；六類 QA 自補 edge case 全綠（含 `limit:-4000` 退款攻擊擋到）；static↔api 逐點比對 5,840 + 2,000 個值零差異（`warnings` 只差排序）；export 372 個 static 檔除 `meta.json` 時戳外 byte-identical、六張 table 逐 row 相同；CUI-0019 線性度同 CUI-0006 時區行為零回歸。新開 **CUI-0029**（🟡 被拒 request 留 traceback，即 S-055）、**CUI-0030**（🟡 四個 view 仲用 raw `error.message`）、**CUI-0031**（🟢 CUI-0024 覆蓋描述主語問題），三個都唔阻結案。**CUI-0027 標 completed 前要先剔 DoD checkbox**（#2 係用戶明文豁免）。上一條：batch review 84 warn → **delta re-review 95 pass**（0 🔴 / 0 🟡 / 3 🟢）；list fan-out per-request row budget 落地，166 aliased `activities` ~2,900 → 161 ms）
