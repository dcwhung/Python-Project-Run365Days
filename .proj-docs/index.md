...
**最後更新**：2026-09-17（**v3.3.0 QA 嘅 9 條 finding 全部清完**：1 🟡（W-061）+ 8 🟢（S-125…S-132）；branch `fix/docs/W-061_qa-findings`（由 `824c682` 開），一個 finding 一個 commit；**588 pytest（+2 = S-129 新測試）/ 144 vitest（25 files）/ TOTAL 96%**（`api/graphql.py`、`src/api/schema.py`、`src/api/service.py`、`src/common/geo.py` 四個都 100%），ruff check + format 全綠、SDL up to date、`npm audit` 全樹 0、`pip-audit` upgrade 後 0、兩個 build mode 綠；S-129 加咗跨 field 次序 gate 並 **mutation-proven ×2**（剷走新 SDL 句 → 紅 2，即新測試兩個 param，其餘 586 全綠 ⇒ 佢係唯一守衛；`activities` widen 做 `list[Activity] | None` → 紅 4）；**四處推翻 QA 報告**（S-130「任何 float 都掟」實測唔成立、S-129 規則係 document 次序唔係 `activities` 有特權、W-061 改為釘死 `99abfcc` 並實跑 `--collect-only` 確認 573、S-127 定位應係另一個 block 而且有三處）；S-131 📝 記錄唔改歷史）<br>_前一次更新_：2026-09-17（**兩份 batch review 嘅 11 條 finding 全部清完**：CUI-0050 批次 86/100 ⚠️ warn（W-050、W-051、S-103…S-106）＋ CUI-0047 批次 90/100 ✅ pass（W-060、S-120…S-124）；branch `fix/review/W-050_batch-findings`，一個 review item 一個 commit；**586 pytest（baseline 573，+13 全部係新 gate）/ 144 vitest（25 files）/ TOTAL 96%**，ruff check + format 全綠、SDL up to date、`npm audit` 全樹 0、`pip-audit` upgrade 後 0、兩個 build mode 綠；W-050 嘅 M6 mutant 獨立重做兩次（修復前 573 綠 + key 真係去到 wire，修復後同一 mutant 紅 9 條），S-104 新增次序 gate 亦 mutation-proven（掉轉 `_page` 兩個 check → 紅 4 條）；W-060 **冇照用** reviewer 建議嘅「約 78%」——實測百分比本身跟計法變（78.63% vs 80.00%），改用「縮減超過 3/4」；S-122 開出 **CUI-0052**（⏸️ 待用戶決定）⇒ ticket census **52 張 / pending 7**）<br>_前一次更新_：2026-09-17（**16 張 ticket 一次過清完 + QA pass**：13 張 batch（CUI-0005/0008/0012/0013/0018/0035…0042）＋ CUI-0043 / 0045 / 0046；四輪 code review（92/80/87/92）＋ batch QA（93/100 pass、0 Critical）；25 條 review finding 全清；**531 pytest / 144 vitest（25 files）/ TOTAL 96%**，`api/schema.py`、`api/service.py`、`api/graphql.py` 都 100%；ticket census 重新對數 = **45 completed / 0 in-progress / 6 pending**；⚠️ `track` 由 `[TrackPoint!]!` 改 `[TrackPoint!]` 係 **breaking change**，下個版本 v3.3.0）
...
| QA 報告（2026-09-17 v3.3.0：六張 ticket + 兩輪 review cleanup）| [`qa/2026-09-17_qa_v3.3.0.md`](qa/2026-09-17_qa_v3.3.0.md)（✅ **pass**、**0 🔴** / 1 🟡（W-061）/ 8 🟢（S-125…S-132）；hard gates **13/13**；lane 報嘅每一個數 QA 都重現到，冇一個要修正；CUI-0047 等價性 30 個輸入對拍、CUI-0048 十五個 `points` 值逐個對數、2 個 mutant 全殺（紅 9 / 紅 4）、6 組主動測試（variable 路徑、`Int` 32-bit 邊界、跨 field 越界、budget+bounds 混合、alias、coercion））<br>✅ **9 條全部已清** —— branch `fix/docs/W-061_qa-findings`，一個 finding 一個 commit；S-129 補咗跨 field 次序測試（mutation-proven ×2）；四處推翻報告（見 `tickets.md` 最後一節）| ✅ 已建立 |
| Code review 報告（2026-09-17 CUI-0050 批次：CUI-0050 / 0048 / 0051）| [`reviews/2026-09-17_review_CUI-0050_batch.md`](reviews/2026-09-17_review_CUI-0050_batch.md)（**86/100 warn**，0 🔴 / 2 🟡（W-050、W-051）/ 4 🟢（S-103…S-106）；hard gates 13/13 pass；lane 五條推翻逐條重做、五條全部成立；6 個 mutant，**第 6 個生還** = W-050（`extensions` 加第三個 key，573 全綠仲去到 wire））<br>✅ **全部 6 條已清** —— branch `fix/review/W-050_batch-findings`，一個 item 一個 commit；W-050 嘅 M6 mutant 由 cleanup lane 獨立重做（修復前 573 綠、修復後紅 9 條），S-104 加咗次序 gate（掉轉 `_page` 兩個 check → 紅 4 條）| ✅ 已建立 |
| Code review 報告（2026-09-17 CUI-0047 批次：CUI-0047 / 0049 / 0044）| [`reviews/2026-09-17_review_CUI-0047_batch.md`](reviews/2026-09-17_review_CUI-0047_batch.md)（**90/100 pass**，0 🔴 / 1 🟡（W-060）/ 5 🟢（S-120…S-124）；hard gates 13/13 pass；等價性 19/19、6 個 mutant 兩方向全殺、真數據 ~388k 座標分量零回歸；部署鐵律用 parsed-YAML 結構比對核實未被郁）<br>✅ **W-060 / S-120 / S-121 / S-122 已清，S-123 / S-124 記錄留檔**；S-122 開出 **CUI-0052**（⏸️ 待用戶決定）| ✅ 已建立 |
