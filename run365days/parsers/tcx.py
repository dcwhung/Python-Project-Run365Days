    ...
    def parse(self, file_path: Path) -> Activity:
        ...
        for lap in activity.findall("ns:Lap", _NS):
            ...
            for trkpt in trk.findall(".//ns:Trackpoint", _NS):
                ...
                lat = (
                    float(pos.find(".//ns:LatitudeDegrees", _NS).text)
                    if pos is not None
                    else None
                )
                lon = (
                    float(pos.find(".//ns:LongitudeDegrees", _NS).text)
                    if pos is not None
                    else None
                )
        ...
        err_pts = sum(1 for tp in track_points if tp.lat is None)
