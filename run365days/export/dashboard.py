...
def total_ascent(elevations: Iterable[Optional[float]], smooth: int = 4) -> float:
...
def merge_temperature(
    tcx_points: List[TrackPoint], gpx_points: Optional[List[TrackPoint]]
) -> Dict[str, float]:
    """Return ``{time: temperature}`` from the GPX track (TCX has no temp)."""
    if not gpx_points:
        return {}
...
def track_rows(activity: Activity, temps: Dict[str, float]) -> List[list]:
    """Compact ``[sec, lat, lon, ele, dist_m, speed, cad, temp]`` rows."""
...
def hourly_at(rows: List[dict], date: str, time_hhmm: str) -> Optional[dict]:
    """Pick the hourly observation closest to *time_hhmm* on *date*."""
...
def payload_to_js(payload: dict) -> str:
    ...
    return f"/* generated by scripts/build_dashboard.py — do not edit */\nwindow.RUN365 = {body};\n"
...
def inline_data(html: str, payload: dict, data_src: str = "data.js") -> str:
    """Replace the ``<script src="data.js">`` tag with the payload inline."""
