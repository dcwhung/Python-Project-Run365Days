...
| `@graphql-codegen/client-preset` 6 唔再產出 schema-wide type | CUI-0049 由 client-preset 4 bump 上 6 之後，`frontend/src/gql/graphql.ts` **縮減超過 3/4**：`Maybe` / `InputMaybe` / `Scalars` 同所有 schema type（`Activity`、`TrackPoint` …）唔再產出，淨低 operation / fragment type 加 `*Document`。⚠️ **下面四個行數刻意唔攞嚟做核對基準**（唔係「唔寫」——S-128 捉到本 row 原文一句「刻意唔寫絕對行數」之後即刻寫咗四個；七項算式逐項實測全部冇錯，錯嘅淨係嗰句措辭）：本 row 原本嘅**主張**係「345 → 69」，`wc -l` 實測係「351 → 75」（喺 `824c682` 量；兩個都細 6，冇計頭 6 行 preamble）——四個數保留低，但只作「點解唔攞行數做基準」嘅理據，唔好攞去 `wc -l` 對數。而且連百分比都跟計法變——`wc -l` 係 78.6%，舊計法係 80.0%，只有 delta（−276 行）兩種計法一致。行數會隨 query 數目同 codegen 版本漂移，同上一 row 嗰個失效嘅 `graphql.ts:318` 係同一種負債。（`f3e2bf5` 個 subject 寫 `drop the codegen line counts from CLAUDE.md section 6`，但 diff 冇 drop 佢哋，係由主張降格成理據；subject 已 commit 唔改得，記喺呢度。）今日只有 `frontend/src/data/api/queries.ts` 用 `@/gql`（而且淨係攞個 `graphql` helper），所以 `tsc -b` 實測零錯；但如果將來想 `import type { Activity } from "@/gql"`，佢已經唔存在，要自己喺 `src/data/types.ts` 寫。另外 `ID` input 而家係 `string | number`（舊版係 `string`） |
