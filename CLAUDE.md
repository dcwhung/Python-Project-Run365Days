...
| `@graphql-codegen/client-preset` 6 唔再產出 schema-wide type | CUI-0049 由 client-preset 4 bump 上 6 之後，`frontend/src/gql/graphql.ts` **縮減超過 3/4**：`Maybe` / `InputMaybe` / `Scalars` 同所有 schema type（`Activity`、`TrackPoint` …）唔再產出，淨低 operation / fragment type 加 `*Document`。⚠️ **呢度刻意唔寫絕對行數**：本 row 原本寫「345 → 69」，`wc -l` 實測係「351 → 75」（兩個都細 6，冇計頭 6 行 preamble）。而且連百分比都跟計法變——`wc -l` 係 78.6%，舊計法係 80.0%，只有 delta（−276 行）兩種計法一致。行數會隨 query 數目同 codegen 版本漂移，同上一 row 嗰個失效嘅 `graphql.ts:318` 係同一種負債。今日只有 `frontend/src/data/api/queries.ts` 用 `@/gql`（而且淨係攞個 `graphql` helper），所以 `tsc -b` 實測零錯；但如果將來想 `import type { Activity } from "@/gql"`，佢已經唔存在，要自己喺 `src/data/types.ts` 寫。另外 `ID` input 而家係 `string | number`（舊版係 `string`） |
